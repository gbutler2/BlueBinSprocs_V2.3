...
Orders
     AS (SELECT Row_number()
         ...
         WHERE  PO_HDR.PO_DT >= FirstScanDate
                AND (LEFT(PO_LN_DST.LOCATION, 2) COLLATE DATABASE_DEFAULT IN (SELECT [ConfigValue] FROM   [bluebin].[Config] WHERE  [ConfigName] = 'REQ_LOCATION' AND Active = 1) 
...
SELECT a.Scanseq,
       ...
       a.OrderType,
       COALESCE(a.CartCountNum, 0) as CartCountNum,
       a.OrderNum,
       a.LineNum,
	   a.OrderUOM,
	   ...
	   1 as Scan,
       CASE
         WHEN Datediff(Day, b.OrderDate, a.OrderDate) < 2 THEN 1
         ELSE 0
       END                     AS HotScan,
       CASE
         WHEN (a.OrderDate < COALESCE(b.CloseDate, GETDATE()) OR CAST(a.OrderDate as DATE) = CAST(b.OrderDate as DATE))
